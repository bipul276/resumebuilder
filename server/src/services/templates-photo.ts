  ...
  const renderSection = (section: SectionType): string => {
    switch (section) {
      ...
      case 'workExperience': return workExperience.length > 0 ? `<div class="section"><div class="section-title">Experience</div>
        ${workExperience.sort((a, b) => a.order - b.order).map((j: WorkExperience) => `<div class="entry"><div class="entry-header"><div><div class="entry-title">${escapeHtml(j.position)}</div><div class="entry-company">${escapeHtml(j.company)}</div></div><div class="entry-date">${formatDate(j.startDate)} – ${j.isCurrentRole ? 'Present' : formatDate(j.endDate)}</div></div>${j.bullets.filter((b: string) => b.trim()).length > 0 ? `<ul class="bullets">${j.bullets.filter((b: string) => b.trim()).map((b: string) => `<li>${escapeHtml(b)}</li>`).join('')}</ul>` : ''}</div>`).join('')}</div>` : '';
      ...
      case 'projects': return projects.length > 0 ? `<div class="section"><div class="section-title">Projects</div>
        ${projects.sort((a, b) => a.order - b.order).map((p: Project) => `<div class="entry"><div class="entry-title">${p.url ? `<a href="${escapeHtml(p.url)}" target="_blank" style="color:inherit;text-decoration:none">${escapeHtml(p.name)}</a>` : escapeHtml(p.name)}</div>${p.description ? `<p style="color:#475569;margin-top:4px">${formatDescription(p.description)}</p>` : ''}</div>`).join('')}</div>` : '';
...
export function renderIdentityTemplate(data: ResumeData): string {
  ...
  const renderSection = (section: SectionType): string => {
    switch (section) {
      ...
      case 'workExperience': return workExperience.length > 0 ? `<div class="section"><div class="section-title">Experience</div>
    ${workExperience.sort((a, b) => a.order - b.order).map((j: WorkExperience) => `<div class="entry"><div class="entry-header"><div><span class="entry-title">${escapeHtml(j.position)}</span> <span class="entry-company">at ${escapeHtml(j.company)}</span></div><span class="entry-date">${formatDate(j.startDate)} – ${j.isCurrentRole ? 'Present' : formatDate(j.endDate)}</span></div>${j.bullets.filter((b: string) => b.trim()).length > 0 ? `<ul class="bullets">${j.bullets.filter((b: string) => b.trim()).map((b: string) => `<li>${escapeHtml(b)}</li>`).join('')}</ul>` : ''}</div>`).join('')}</div>` : '';
      case 'education': return education.length > 0 ? `<div class="section"><div class="section-title">Education</div>
