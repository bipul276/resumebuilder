...
export function renderElegantTemplate(data: ResumeData): string {
  ...
  const renderSection = (section: SectionType): string => {
    ...
    ${projects.sort((a, b) => a.order - b.order).map((p: Project) => `<div class="entry" style="text-align:center"><span class="entry-title">${p.url ? `<a href="${escapeHtml(p.url)}" target="_blank" style="color:inherit;text-decoration:none">${escapeHtml(p.name)}</a>` : escapeHtml(p.name)}</span>${p.description ? `<p style="color:#4b5563;margin-top:4px">${formatDescription(p.description)}</p>` : ''}</div>`).join('')}</div>` : '';
...
export function renderBoldTemplate(data: ResumeData): string {
  ...
  const renderSection = (section: SectionType): string => {
    ...
    ${education.sort((a, b) => a.order - b.order).map((e: Education) => `<div class="entry"><span class="entry-title">${escapeHtml(e.degree)}</span> - ${escapeHtml(e.institution)} (${formatDate(e.endDate)})</div>`).join('')}</div>` : '';
      case 'projects': return projects.length > 0 ? `<div class="section"><div class="section-title">Projects</div>
    ${projects.sort((a, b) => a.order - b.order).map((p: Project) => `<div class="entry"><span class="entry-title">${p.url ? `<a href="${escapeHtml(p.url)}" target="_blank" style="color:inherit;text-decoration:none">${escapeHtml(p.name)}</a>` : escapeHtml(p.name)}</span>${p.description ? `<p style="margin-top:4px">${formatDescription(p.description)}</p>` : ''}</div>`).join('')}</div>` : '';
      case 'certifications': return renderCertificationsHtml(data);
      default:
        if (typeof section === 'string' && section.startsWith('custom_')) return renderCustomSectionHtml(data, section);
        return '';
...
export function renderSimpleTemplate(data: ResumeData): string {
  ...
  const renderSection = (section: SectionType): string => {
    switch (section) {
      ...
      case 'workExperience': return workExperience.length > 0 ? `<div class="section"><div class="section-title">Experience</div>
    ${workExperience.sort((a, b) => a.order - b.order).map((j: WorkExperience) => `<div class="entry"><div class="entry-header"><span><span class="entry-title">${escapeHtml(j.position)}</span> - ${escapeHtml(j.company)}</span><span class="entry-date">${formatDate(j.startDate)} - ${j.isCurrentRole ? 'Present' : formatDate(j.endDate)}</span></div>${j.bullets.filter((b: string) => b.trim()).length > 0 ? `<ul class="bullets">${j.bullets.filter((b: string) => b.trim()).map((b: string) => `<li>${escapeHtml(b)}</li>`).join('')}</ul>` : ''}</div>`).join('')}</div>` : '';
      case 'education': return education.length > 0 ? `<div class="section"><div class="section-title">Education</div>
    ${education.sort((a, b) => a.order - b.order).map((e: Education) => `<div class="entry"><div class="entry-header"><span class="entry-title">${escapeHtml(e.degree)}${e.field ? ` - ${escapeHtml(e.field)}` : ''}</span><span class="entry-date">${formatDate(e.endDate)}</span></div><div>${escapeHtml(e.institution)}</div></div>`).join('')}</div>` : '';
      ...
      case 'projects': return projects.length > 0 ? `<div class="section"><div class="section-title">Projects</div>
    ${projects.sort((a, b) => a.order - b.order).map((p: Project) => `<div class="entry"><span class="entry-title">${p.url ? `<a href="${escapeHtml(p.url)}" target="_blank" style="color:inherit;text-decoration:none">${escapeHtml(p.name)}</a>` : escapeHtml(p.name)}</span>${p.description ? ` - ${formatDescription(p.description)}` : ''}</div>`).join('')}</div>` : '';
      case 'certifications': return renderCertificationsHtml(data);
      default:
        if (typeof section === 'string' && section.startsWith('custom_')) return renderCustomSectionHtml(data, section);
        return '';
...
export function renderTimelineTemplate(data: ResumeData): string {
  ...
  const renderSection = (section: SectionType): string => {
    ...
    ${workExperience.sort((a, b) => a.order - b.order).map((j: WorkExperience) => `<div class="timeline-entry"><div class="entry-date">${formatDate(j.startDate)} – ${j.isCurrentRole ? 'Present' : formatDate(j.endDate)}</div><div class="entry-title">${escapeHtml(j.position)}</div><div class="entry-company">${escapeHtml(j.company)}</div>${j.bullets.filter((b: string) => b.trim()).length > 0 ? `<ul class="bullets">${j.bullets.filter((b: string) => b.trim()).map((b: string) => `<li>${escapeHtml(b)}</li>`).join('')}</ul>` : ''}</div>`).join('')}</div></div>` : '';
    ...
    ${projects.sort((a, b) => a.order - b.order).map((p: Project) => `<div class="timeline-entry"><div class="entry-title">${p.url ? `<a href="${escapeHtml(p.url)}" target="_blank" style="color:inherit;text-decoration:none">${escapeHtml(p.name)}</a>` : escapeHtml(p.name)}</div>${p.description ? `<div class="entry-company">${formatDescription(p.description)}</div>` : ''}</div>`).join('')}</div></div>` : '';
      case 'certifications': return renderCertificationsHtml(data);
      default:
        if (typeof section === 'string' && section.startsWith('custom_')) return renderCustomSectionHtml(data, section);
        return '';
