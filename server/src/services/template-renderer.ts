  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    html, body {
      ...
      color: var(--text);
      background: #fff;
      padding: ${getMarginStyle(settings?.margin, 15)};
    }
    
    @page { size: A4; margin: 0; }
    @media print {
      body { padding: ${getMarginStyle(settings?.margin, 15)}; min-height: auto; height: auto; }
    }
...
<body>
  <header class="header">
    ...
    <div class="contact">
      ...
      personalInfo.linkedIn ? `<span><a href="${escapeHtml(ensureHttps(personalInfo.linkedIn))}" target="_blank">${(settings?.useFullUrls ?? true) ? escapeHtml(formatDisplayUrl(personalInfo.linkedIn)) : 'LinkedIn'}</a></span>` : '',
      personalInfo.github ? `<span><a href="${escapeHtml(ensureHttps(personalInfo.github))}" target="_blank">${(settings?.useFullUrls ?? true) ? escapeHtml(formatDisplayUrl(personalInfo.github)) : 'GitHub'}</a></span>` : '',
      personalInfo.portfolio ? `<span><a href="${escapeHtml(ensureHttps(personalInfo.portfolio))}" target="_blank">${(settings?.useFullUrls ?? true) ? escapeHtml(formatDisplayUrl(personalInfo.portfolio)) : 'Portfolio'}</a></span>` : ''
    ].filter(Boolean).join(' • ')}
    </div>
  </header>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .sidebar { background: var(--bg-sidebar); padding: ${getMarginStyle(settings?.margin, 15)}; border-right: 1px solid var(--border-color); }
    .main-content { padding: ${getMarginStyle(settings?.margin, 15)}; }
